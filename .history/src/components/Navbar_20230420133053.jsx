...
const Navbar = () => {
  const navbarButtons = [
    { name: "Home", menu: false, items: [], link: "/" },
    {
      name: "Events",
      menu: true,
    ...
    { name: "Animals", menu: false, items: [], link: "/Animals" },
    { name: "News", menu: false, items: [], link: "/News" },
  ];
  ...
  return (
    <>
      <div className={navbar.navbar}>
            ...
            {navbarButtons.map((item, n) => (
              <div style={{ display: "block", overflow: "visible" }} key={n}>
                <div className={navbar.navbarButton}>
                  <Link to={item.link} style={{ textDecoration: "none", color: "unset" }}>
                    {item.name + (item.menu ? " ⯆" : "")}
                  </Link>
                </div>
                ...
                {item.items.length > 0 && (
                  <div className={navbar.navbarButtonDropdown}>
                    {item.items.map((i, n) => (
                      <div className={navbar.navbarButtonDropdownItem} key={n}>
                        {i}
                      </div>
                    ))}
                  </div>
        ...
        <img className={navbar.mobileButton} src={hamburger} onClick={() => mobileMenu === true ? toggleMobileMenu(false) : toggleMobileMenu(true)} alt="" />
        {mobileMenu && <div className={navbar.mobileMenu}></div>}
      </div>
    </>
  );
};
